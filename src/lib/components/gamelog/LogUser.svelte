<script lang="ts">
	...
	function getFilteredTags(tags: any) {
		if (!tags) return [];

		const prioritizedTag = tags.reduce((highest: any, tag: any) => {
	...
	function openUserProfile() {
		open(`https://vrchat.com/home/user/${logData.user}`);
	}

	onMount(async () => {
		let userData = await invoke<string>('get_vrc_user', { userId: logData.user });
